...
class WoocommerceIntegration {
    ...
    formatCustomerData(wcCustomer) {
        const customer = {
            id: wcCustomer.id,
            name: `${wcCustomer.first_name || ''} ${wcCustomer.last_name || ''}`.trim() || wcCustomer.username || `مشتری ${wcCustomer.id}`,
        ...
        console.log(`👤 Formatted customer: ${customer.name}`, customer);
        return customer;
    }
    ...
    formatProductData(wcProduct) {
        const product = {
            ...
            name: wcProduct.name,
            stock: wcProduct.stock_quantity !== null && wcProduct.stock_quantity !== undefined ? wcProduct.stock_quantity : Infinity,
            price: Math.round((parseFloat(wcProduct.price) || 0) * 10), // تبدیل به ریال
        ...
        console.log(`📦 Formatted product: ${product.name}`, product);
        return product;
    }
    ...
    async saveCustomersToSystem(wcCustomers) {
        try {
            ...
            wcCustomers.forEach(wcCustomer => {
                const existingIndex = existingCustomers.findIndex(c => c.woocommerceId === wcCustomer.woocommerceId);
                ...
                if (existingIndex >= 0) {
                    ...
                    // افزودن مشتری جدید
                    existingCustomers.push(wcCustomer);
                }
            });
            ...
            console.log(`💾 Saved ${wcCustomers.length} customers to system`);
    ...
    async saveProductsToSystem(wcProducts) {
        try {
            ...
            wcProducts.forEach(wcProduct => {
                const existingIndex = existingProducts.findIndex(p => p.woocommerceId === wcProduct.woocommerceId);
                ...
                if (existingIndex >= 0) {
                    ...
                    // افزودن محصول جدید
                    existingProducts.push(wcProduct);
                }
            });
            ...
            console.log(`💾 Saved ${wcProducts.length} products to system`);
