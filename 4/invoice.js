...
// تابع بهبود یافته جستجوی محصولات
function updateProductSearch() {
    ...
    // ایجاد هدر برای dropdown
    if (!dropdown.querySelector('.dropdown-header')) {
        dropdown.innerHTML = `
            <div class="dropdown-header bg-gray-100 p-3 border-b flex justify-between items-center rounded-t-lg">
                <span class="font-semibold text-gray-700">نتایج جستجو</span>
                <button onclick="closeProductDropdown()" class="text-gray-500 hover:text-gray-700 text-lg font-bold">
                    &times;
                </button>
            </div>
            <div class="dropdown-body max-h-60 overflow-y-auto"></div>
        `;
    }
    ...
    searchInput.addEventListener('input', () => {
        ...
        // جستجو در محصولات محلی و ووکامرس
        ...
        filteredProducts.forEach((product, index) => {
            ...
            div.className = 'dropdown-item p-3 border-b hover:bg-gray-50 cursor-pointer transition';
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-start mb-1">
                            ...
                            <span class="text-green-600 font-semibold text-sm bg-green-50 px-2 py-1 rounded">
                        ...
                        <div class="flex items-center gap-4 text-xs text-gray-500 mb-1">
                            ...
                            <span>موجودی: ${product.stock === Infinity ? 'نامحدود' : toPersianDigits(product.stock)}</span>
                            ${product.source === 'woocommerce' ? '<span class="bg-green-100 text-green-800 px-2 py-1 rounded">ووکامرس</span>' : ''}
                        </div>
            ...
            div.addEventListener('click', (e) => {
                e.stopPropagation();
                selectProductForInvoice(productIndex, product);
                dropdown.classList.remove('show');
            });
...
// تابع انتخاب محصول برای فاکتور
function selectProductForInvoice(productIndex, product) {
    document.getElementById('itemSearch').value = product.name;
    document.getElementById('itemCode').value = productIndex;
    document.getElementById('itemName').value = product.name;
    document.getElementById('itemPrice').value = product.price;
    
    // نمایش تصویر محصول در پیش‌نمایش
    const imagePreview = document.getElementById('itemImagePreview');
    if (imagePreview) {
        if (product.image) {
            imagePreview.innerHTML = `
                <div class="bg-white p-4 rounded-lg border shadow-sm">
                    <div class="flex justify-between items-start mb-3">
                        <h4 class="font-semibold text-gray-800">پیش‌نمایش محصول</h4>
                        <button onclick="clearProductPreview()" class="text-gray-400 hover:text-gray-600">
                            <i class="fas fa-times"></i>
                        </button>
                    </div>
                    <img src="${product.image}" 
                         alt="${product.name}" 
                         class="rounded-lg shadow-md max-w-full h-auto mx-auto max-h-48 object-contain"
                         onerror="this.style.display='none'">
                    <div class="mt-3 text-center">
                        <div class="font-medium">${product.name}</div>
                        <div class="text-sm text-gray-600">${formatPrice(product.price)} ریال</div>
                    </div>
                </div>
            `;
...
// تابع بهبود یافته برای افزودن آیتم به فاکتور
function addItem() {
    ...
    if (codeIndex !== '') {
        ...
        if (product && product.stock !== Infinity && product.stock < qty) {
            showNotification(`موجودی کافی نیست! موجودی فعلی: ${toPersianDigits(product.stock)} عدد`, 'error');
            return;
        }
    }
    ...
    if (existingItemIndex >= 0) {
        ...
        items[existingItemIndex].total = items[existingItemIndex].qty * items[existingItemIndex].price;
        ...
        items.push({
            ...
            productIndex: codeIndex !== '' ? parseInt(codeIndex) : null
        });
    ...
    showNotification('آیتم با موفقیت به فاکتور افزوده شد.', 'success');
}

// تابع بهبود یافته برای به‌روزرسانی جدول آیتم‌ها
function updateTable() {
    ...
    if (items.length === 0) {
        table.innerHTML = `
            <tr>
                <td colspan="9" class="p-8 text-center text-gray-500">
                    ...
                    <p>هیچ آیتمی در فاکتور وجود ندارد</p>
                    <p class="text-sm mt-2">از قسمت جستجوی بالا محصولات را اضافه کنید</p>
                </td>
            </tr>
        `;
    } else {
        items.forEach((item, index) => {
            ...
            row.innerHTML = `
                <td class="p-3 border text-center">
                    <span class="bg-blue-100 text-blue-800 rounded-full w-8 h-8 flex items-center justify-center text-sm font-medium mx-auto">
                ...
                <td class="p-3 border text-center">
                    ${item.image ? `
                        <div class="relative group">
                            <img src="${item.image}" 
                                 class="w-12 h-12 object-cover rounded-lg border mx-auto cursor-pointer"
                                 onclick="showImageModal('${item.image}', '${item.name}')"
                                 onerror="this.style.display='none'">
                ...
                <td class="p-3 border text-center font-mono text-sm">
                    ${item.code || '<span class="text-gray-400">-</span>'}
                </td>
                
                <td class="p-3 border">
                    <div class="flex flex-col">
                        <span class="font-semibold text-gray-800 text-right">${item.name}</span>
                        ${item.code ? `<span class="text-xs text-gray-500 text-right">کد: ${item.code}</span>` : ''}
                    </div>
                </td>
                ...
                <td class="p-3 border text-left font-mono text-sm">
                    ${priceDisplay}
                    <div class="text-xs text-gray-500">${currency === 'rial' ? 'ریال' : 'تومان'}</div>
                </td>
                
                <td class="p-3 border text-left font-mono text-sm font-semibold">
                    ${totalDisplay}
                    <div class="text-xs text-gray-500">${currency === 'rial' ? 'ریال' : 'تومان'}</div>
                </td>
        ...
        summaryRow.innerHTML = `
            <td class="p-3 border text-center" colspan="4">
                <span class="text-gray-700">جمع کل</span>
            </td>
            <td class="p-3 border text-center">
                <span class="text-blue-600">${toPersianDigits(totalQty)}</span>
            </td>
            <td class="p-3 border text-center">
                <span class="text-gray-600">-</span>
            </td>
            <td class="p-3 border text-left">
                <span class="text-green-600">${formatPrice(subTotal)}</span>
                <div class="text-xs text-gray-500">${currency === 'rial' ? 'ریال' : 'تومان'}</div>
            </td>
            <td class="p-3 border text-center" colspan="2">
                <span class="text-gray-600">-</span>
            </td>
        `;
...
// تابع بهبود یافته برای پاک کردن فیلدهای آیتم
...
// تابع بهبود یافته برای ویرایش آیتم
function editItem(index) {
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-lg w-full max-w-md">
            <div class="p-4 border-b flex justify-between items-center">
                <h3 class="font-semibold text-lg">ویرایش آیتم</h3>
            ...
            <div class="p-4">
                <div class="mb-4">
                    <label class="block text-gray-700 mb-2">نام محصول</label>
                ...
                <div class="grid grid-cols-2 gap-4 mb-4">
                    <div>
                        <label class="block text-gray-700 mb-2">تعداد</label>
                        <input type="number" id="editItemQty" value="${item.qty}" min="1"
                               class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-gray-700 mb-2">قیمت واحد</label>
                        <input type="number" id="editItemPrice" value="${item.price}" min="0"
                               class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
                ...
                ${item.image ? `
                    <div class="mb-4">
                        <label class="block text-gray-700 mb-2">تصویر محصول</label>
                        <img src="${item.image}" class="w-20 h-20 object-cover rounded-lg border mx-auto">
                    </div>
                ` : ''}
...
function saveItemEdit(index) {
    ...
    // بازگرداندن موجودی قبلی اگر محصول از انبار بوده
    if (oldItem.productIndex !== null && products[oldItem.productIndex]) {
        const product = products[oldItem.productIndex];
    ...
    showNotification('آیتم با موفقیت ویرایش شد.', 'success');
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    loadCustomers();
    loadProducts();
    updateProductSearch(); // فعال کردن جستجوی پیشرفته محصولات
});
...
// invoice.js - بهبود جستجوی خریدار و آیتم‌ها برای نمایش داده‌های ووکامرس

// تابع بهبود یافته جستجوی خریدار
function updateCustomerSearch() {
    ...
    searchInput.addEventListener('input', () => {
        ...
        // جستجو در مشتریان محلی و ووکامرس
        ...
        filteredCustomers.forEach((customer, index) => {
            ...
            div.innerHTML = `
                <div>
                    ...
                    ${customer.nationalId ? `<span class="text-gray-500 text-sm mr-2">(${toPersianDigits(customer.nationalId)})</span>` : ''}
                </div>
            ...
            div.addEventListener('click', () => {
                document.getElementById('customerSearch').value = `${customer.name} ${customer.nationalId ? `(${customer.nationalId})` : ''}`;
...
// تابع نمایش اطلاعات مشتری انتخاب شده
function showSelectedCustomerInfo(customer) {
    // ایجاد یا به‌روزرسانی بخش نمایش اطلاعات مشتری
    let infoContainer = document.getElementById('selectedCustomerInfo');
    if (!infoContainer) {
        infoContainer = document.createElement('div');
        infoContainer.id = 'selectedCustomerInfo';
        infoContainer.className = 'mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200';
        
        const searchContainer = document.getElementById('customerSearch').parentNode;
        searchContainer.parentNode.insertBefore(infoContainer, searchContainer.nextSibling);
    }
    ...
    infoContainer.innerHTML = `
        <div class="flex justify-between items-start mb-2">
            <h4 class="font-semibold text-blue-800">مشتری انتخاب شده</h4>
            ${woocommerceBadge}
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-sm">
            <div><span class="text-gray-600">نام:</span> ${customer.name}</div>
            ${customer.phone ? `<div><span class="text-gray-600">تلفن:</span> ${toPersianDigits(customer.phone)}</div>` : ''}
            ${customer.email ? `<div><span class="text-gray-600">ایمیل:</span> ${customer.email}</div>` : ''}
            ${customer.nationalId ? `<div><span class="text-gray-600">کد ملی:</span> ${toPersianDigits(customer.nationalId)}</div>` : ''}
            ${customer.address ? `<div class="sm:col-span-2"><span class="text-gray-600">آدرس:</span> ${customer.address}</div>` : ''}
        </div>
    `;
}

// تابع نمایش اطلاعات محصول انتخاب شده
function showSelectedProductInfo(product) {
    let infoContainer = document.getElementById('selectedProductInfo');
    if (!infoContainer) {
        infoContainer = document.createElement('div');
        infoContainer.id = 'selectedProductInfo';
        infoContainer.className = 'mt-4 p-4 bg-green-50 rounded-lg border border-green-200';
        
        const searchContainer = document.getElementById('itemSearch').parentNode;
        searchContainer.parentNode.insertBefore(infoContainer, searchContainer.nextSibling);
    }
    
    const woocommerceBadge = product.source === 'woocommerce' ? 
        '<span class="bg-green-100 text-green-800 text-xs px-2 py-1 rounded mr-2">محصول ووکامرس</span>' : '';
    ...
    infoContainer.innerHTML = `
        <div class="flex justify-between items-start mb-2">
            <h4 class="font-semibold text-green-800">محصول انتخاب شده</h4>
            ${woocommerceBadge}
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div class="flex items-start">
                ${product.image ? `<img src="${product.image}" class="w-16 h-16 object-cover rounded-lg ml-4">` : ''}
                <div>
                    <div class="font-medium">${product.name}</div>
                    <div class="text-sm text-gray-600">کد: ${product.code}</div>
                </div>
            </div>
            <div class="space-y-2 text-sm">
                <div><span class="text-gray-600">قیمت:</span> ${formatPrice(product.price)} ریال</div>
                <div><span class="text-gray-600">موجودی:</span> ${stockStatus}</div>
                ${product.woocommerceId ? `<div><span class="text-gray-600">ID ووکامرس:</span> ${toPersianDigits(product.woocommerceId)}</div>` : ''}
            </div>
        </div>
        ${product.description ? `
            <div class="mt-3 pt-3 border-t border-green-200">
                <div class="text-sm text-gray-600">توضیحات:</div>
                <div class="text-sm mt-1">${product.description}</div>
            </div>
        ` : ''}
    `;
}